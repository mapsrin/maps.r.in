...
labsPD <- lapply(seq(nrow(states@data)), function(i) {
  paste0( '<p>', sprintf(
        ...
        states@data[i, "% of Total Population"], 100.0 - states@data[i, "% of Total Population"],
        format(states@data[i, "Area (sqkm)"], big.mark=","), 
        format(round(100.0*states@data[i, "Area (sqkm)"]/3287240,2)),
        100.0*states@data[i, "Area (sqkm)"]/3287240, 100.0 - 100.0*states@data[i, "Area (sqkm)"]/3287240,
...
map <- leaflet(width = "100%") %>% 
  ...
  addLegend("bottomleft", pal = palpopdens, values = plotSubset$`Density (per sqkm)`, title = "Population Density", opacity = 1, group = "Population Density", layerId = "Population Density" )
...
```{r, echo=FALSE, warning=F}
labsPG <- lapply(seq(nrow(states@data)), function(i) {
  paste0( '<p>', sprintf(
        "<div style='width:95%%'>
        <div style='font-size:12px;width:200px;float:left'>
        <span style='font-size:12px;font-weight:bold'>%s</span><hr>
...
map <- leaflet(width = "100%") %>% 
  ...
  addPolygons(data = states,
              ...
              opacity = 1.0, fillOpacity = 0.8,
              dashArray = "3",
              fillColor = ~palpopgrwth(states$`Decadal Growth%(2001-2011)`),
              label = lapply(labsPG, htmltools::HTML),
              labelOptions = labelOptions(
                offset=c(-100,-140),
                textOnly = T,
                style=list(
  ...
  addScaleBar() %>%
  addSimpleGraticule(showOriginLabel = TRUE) %>%
  addLegend("bottomleft", pal = palpopgrwth, values = plotSubset$`Decadal Growth%(2001-2011)`, title = "Population Growth %", opacity = 1, group = "Population Growth %", layerId = "Population Growth %" )
...
```{r, echo=FALSE, warning=F}
labsLit <- lapply(seq(nrow(states@data)), function(i) {
  paste0( '<p>', sprintf(
        "<div style='width:95%%'>
        <div style='font-size:12px;width:200px;float:left'>
        <span style='font-size:12px;font-weight:bold'>%s</span><hr>
...
map <- leaflet(width = "100%") %>% 
  ...
  addPolygons(data = states,
              ...
              opacity = 1.0, fillOpacity = 0.8,
              dashArray = "3",
              fillColor = ~palpopLit(states$`Literacy Rate (%)`),
              label = lapply(labsLit, htmltools::HTML),
              labelOptions = labelOptions(
                offset=c(-100,-140),
                textOnly = T,
                style=list(
  ...
  addScaleBar() %>%
  addSimpleGraticule(showOriginLabel = TRUE) %>%
  addLegend("bottomleft", pal = palpopLit, values = plotSubset$`Literacy Rate (%)`, title = "Literacy Rate %", opacity = 1, group = "Literacy Rate %", layerId = "Literacy Rate %" )
...
mapGR <- leaflet(width = "100%") %>%
  ...
  addPolygons(
    ...
    opacity = 1.0, fillOpacity = 0.8,
    dashArray = "3",
    fillColor = ~palpopgr(as.numeric(states$`Gender Ratio`)),
  ...
  addScaleBar() %>%
  addSimpleGraticule(showOriginLabel = TRUE) %>%
  legendreverse("bottomleft", pal = palpopgr, values = plotSubset$`Gender Ratio`, opacity = 1, decreasing = TRUE, title = "Gender Ratio", group = "Gender Ratio", layerId = "Gender Ratio" )
...
mapUR <- leaflet(width = "100%") %>%
  ...
  addScaleBar() %>%
  addSimpleGraticule(showOriginLabel = TRUE) %>%
  legendreverse("bottomleft", pal = palpopur, values = as.numeric(plotSubset$`Urban Rural Ratio`), decreasing = T, opacity = 1, title = "Urban:Rural", group = "Urban:Rural", layerId = "Urban:Rural" )
