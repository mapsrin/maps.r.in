...
hoverlabels <- lapply(seq(nrow(districts@data)), function(i) {
  ...
  paste0( '<p>', sprintf(
        ...
        <div style='width:95%%'>
        <span style='float:left'>NMR (/1000):</span>
...
```
## {.tabset}
### India, Population Growth vs Neonatal Mortality Rate^[1]^
...
# # save SVG legend
ggsave("data/img/districtsPopGrNMRLegend.svg", plot = legend,
...
map <- leaflet(width = "100%") %>% 
  ...
  addControl(
    "<img src = 'data/img/districtsPopGrNMRLegend.svg' width = '128' height = '128'>",
    position = "bottomleft",
...
<h2><b>India</b></h2>
<div id = "countryInfo"><p><div style='width:95%%'><div style='font-size:12px;width:200px;float:left'><span style='float:left'>Total NMR (/1000):</span><span style='float:right'> 30.5</span><br/><span style='float:left'>Population Growth (%):</span><span style='float:right'> 9.21</span><br/><br/></div></div></p>
</div>
<div id = "districtInfo">
</div>
</div>
...
layout(margin=list(t = 75), title = "Neonatal Mortality Rate (/1000) vs Population Growth (%)", xaxis = list(title = "NMR (/1000)"), yaxis = list(title = "Population Growth (%)")) %>% 
add_annotations(text = c("[33rd]","[66th]"),  x = c(brksVar1$brks[2],brksVar1$brks[3]), y=1, showarrow=F,textangle=-90, valign="top", yref="paper", xanchor="right", yanchor="top") %>%
add_annotations(text = c("[33rd]","[66th]"),  x = 1, y=c(brksVar2$brks[2],brksVar2$brks[3]), showarrow=F, valign="top", xref="paper", xanchor="right", yanchor="bottom") %>%
add_annotations(text = rp, showarrow=F, x=1, y=1, xref="paper", yref="paper",xanchor="right", yanchor="top") %>%
hide_colorbar() %>%  layout(showlegend = F)
```
<font size='1'>
